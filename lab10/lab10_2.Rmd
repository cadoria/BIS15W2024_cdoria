...
penguins %>%
  ...
  geom_col() #could still use geom_bar but it wont display the numerical info and wouldnt need to include y = 
```
...
```{r}
penguins %>%
  filter(sex!= "NA") %>% #puls out the NAs
  group_by(sex) %>% 
...
```{r}
homerange %>% #did not have to use groupby
