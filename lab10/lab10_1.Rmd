...
ggplot(data=homerange, mapping=aes(x=log10.mass, y=log10.hra))+
  geom_jitter() #adds some random noise to separate things out a little bit
```
...
```{r}
homerange %>% 
  ggplot(aes(x=trophic.guild)) + #dont even have to do x equals bc there is only one variable in the plot, does not work with continuos data
...
homerange %>% 
  ...
  geom_col()
  #coord_flip() flips aesthetic only
```
