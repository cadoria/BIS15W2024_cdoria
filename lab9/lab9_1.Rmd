...
Patient will not move, the names of the columns to a new column called drug and move the value to a new column called heartrate.
```{r}
heartrate %>% 
...
```{r}
names(relig_income)
#the column names are observations, money as the data columns
```
...
billboard3 <- 
  billboard %>% 
  pivot_longer(-c(artist, track, date.entered), #specific columns to stay fixed, move everything except for these
               names_to = "week",
               values_to = "rank",
               values_drop_na = TRUE
               )
...
The data are not tidy because the days are observations but they are set as individual columns. 
```{r}
...
qpcr_untidy %>% 
  pivot_longer(
    exp1_rep1:exp3_rep3,
    names_to= c("experiment", "replicate"), #used to name the new variable
    names_sep="_", #separates by the underscore
    values_to="mRNA_expression" #used to move the vales to a new column names this
  )
```
