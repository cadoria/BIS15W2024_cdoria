...
#we need an x and y variable, this is not tidy enough to make a plot
```
...
expenditures %>% 
  pivot_longer(-expenditure, #not moving expenditure
               names_to = "year", #naming a new column varible year
               values_to = "bn_dollars") # moving the values into billio dollar
```
...
Restrict the data to private education only. Sort in ascending order.
```{r}
expenditures %>% 
  pivot_longer(-expenditure,
               names_to = "year",
...
The opposite of `pivot_longer()`. You use `pivot_wider()` when you have an observation scattered across multiple rows. In the example below, `cases` and `population` represent variable names not observations.  
 observations scattered across lots of rows
Rules:  
