...
#list apply, looks at the files and iterates read_cvs over every file found
```

We can view elements in our list with double brackets. Let's view the data for Butte county.
```{r}
spider_list[[3]]
#look at third element in the list
```
...
```{r}
head(spider_list [[4]]) #must use double brackets
```
