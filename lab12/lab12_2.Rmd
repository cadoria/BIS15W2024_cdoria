...
```{r}
q <- homerange %>%
  ggplot(aes(x= log10.mass, y = log10.preymass, color= taxon))+ #fill wont work on a scatter plot need to use color
  geom_point(na.rm = T)
q
```
...
```{r}
q+theme_classic() #can change the aesthetics to minial or wtv
```
...
p+theme_linedraw()+
  ...
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL, #removes the label form the x axis
       y= "n",
       fill= "Trophic Guild")
```
...
```{r}
display.brewer.pal(7,"YlOrBr") #sequential palette
```
...
+`scale_colour_brewer()` is for points  for geom point
+`scale_fill_brewer()` is for fills  dont fill in points
...
```{r}
my_palette <- paletteer_d("werpals::monet") #store your palette
