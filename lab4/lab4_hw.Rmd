...
```{r}
mean(only_carnivores$"mean.hra.m2", na.rm = T)
```

```{r}
mean(only_herbivores$"mean.hra.m2", na.rm = T)
...
```
```{r}
owls <- filter(homerange, )
```


```{r}
homerange_subset <- select(homerange, "mean.mass.g", "log10.mass", "family", "genus", "species")
homerange_subset
```
