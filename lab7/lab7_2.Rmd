...
```{r}
glimpse(penguins)
#categorical = not number. group_by only works for categorical??
```
...
#tells us how many islands. island shows as a factor
```
...
#tells us how many nas are found in each island
```
...
penguins %>% 
  ...
  summarize(total=n(), .groups= 'keep')#the .groups argument here just prevents a warning message
```
...
```{r}
penguins %>% 
  count(island) #whats the difference?
```
...
```{r}
penguins %>%
  summarize(across(c(species, island, sex), n_distinct)) #n_distinct counts the number of distinct observations
```
...
```{r}
penguins %>%
  summarize(across(contains("mm"), \(x) mean(x, na.rm = TRUE))) #this one gets rid of the error that happens above. 
```
